...
IAffichageGraphique::IAffichageGraphique()
...
bool IAffichageGraphique::demanderStand() {
    return false;
}

void IAffichageGraphique::afficherMain(const ListeChaine <Carte> &p_main) {
    int nbCarteMax = 7;
    ...
    int nombreDeCartesDansLaMain = p_main.nombreElements();

    for (int i = 0; i < nombreDeCartesDansLaMain; ++i) {
        if (nbCarteAfficher < nbCarteMax) {
            Carte carte = p_main.obtenir(i);
...
void IAffichageGraphique::afficherCarteDansLabel(std::string p_cheminCarte, TGUIWSFML::Component::Label *label) {
...
void IAffichageGraphique::afficherScore(int p_score) {
    this->m_score->text("Score : " + std::to_string(p_score));
}

void IAffichageGraphique::afficherMessage(const std::string &p_message) {
    this->m_message->text(p_message);
}

void IAffichageGraphique::afficherEtatJeuJoueur(Joueur &p_joueur) {
    afficherMain(p_joueur.obtenirMain());
    afficherScore(p_joueur.calculerScore());
}

void IAffichageGraphique::afficherEtatJeuCroupier(Croupier &p_croupier) {
    afficherMain(p_croupier.obtenirMain());
    afficherScore(p_croupier.calculerScore());
}



//void IAffichageGraphique::lancerInterfaceGraphique(Blackjack* p_partieBlackjack)
//{
//	p_partieBlackjack->jouerMancheConsole(); // Ref circulaire
//}

void IAffichageGraphique::componentsInitialization() {
...
bool IAffichageGraphique::gererHitStand() {
    while (!this->aClick) {
...
void IAffichageGraphique::hit() {
    //Lignes suivante simplement pour illustrer le fonctionnement de mes foncitons pour le GUI
    this->m_solde->text("Solde : 100");
    this->m_mise->text("Mise : 10");
    afficherCarteDansLabel("hearts_king", this->m_imageCarte7);
    afficherScore(10);
    afficherMessage("Vous pigez une carte");

    this->hitOuStand = true;
    this->aClick = true;
}

void IAffichageGraphique::stand() {
    this->hitOuStand = false;
    this->aClick = true;
}

void IAffichageGraphique::nouvellePartie() {
    // TODO
}
